#!/usr/bin/env python3
"""Apply the Phase 17 authenticated context and durable result-store slice.

This script is intentionally idempotent. It is used by the branch-only builder
workflow because the chat execution sandbox cannot clone GitHub directly.
"""
...
def regex_once(path: str, pattern: str, replacement: str) -> None:
    ...
    if count == 0:
        if replacement in text:
            return
        raise RuntimeError(f"regex anchor not found in {path}: {pattern}")
    write(path, updated)
...
enum TerminalOutcome {
  ...
  TERMINAL_OUTCOME_CANCELED = 3;
  TERMINAL_OUTCOME_TIMED_OUT = 4;
  TERMINAL_OUTCOME_REJECTED = 5;
}
...
message TaskResultEnvelope {
  ...
  string executor_peer_id = 5;
  int64 duration_ms = 6;
  int64 completed_at_ms = 7;
  string error_reason = 8;
  map<string, string> result_metadata = 9;
  repeated ResultArtifact output_artifacts = 10;
}
'''

RESULTS_RS = r'''//! Durable authenticated task context, terminal results, and result-delivery outbox.
...
fn make_outbox(result: &TerminalResultRecord) -> Option<ResultOutboxRecord> {
    result.return_peer_id.as_ref().map(|target| ResultOutboxRecord {
...
impl InMemoryStore {
    ...
    pub fn complete_task_with_result(
        ...
        if let Some(outbox) = make_outbox(&result) {
            state.result_outbox.insert(outbox.delivery_id.clone(), outbox);
        }
...
impl SqliteStore {
    pub async fn accept_task_with_envelope_and_context(
        ...
        if let Some(existing) = fetch_task_optional_with_executor(&mut tx, task.task_id()).await? {
            ...
            let existing_context = fetch_transport_context_optional(&mut tx, task.task_id()).await?;
            if existing == task
    ...
    pub async fn get_terminal_result(
        &self,
        task_id: &TaskId,
    ) -> StoreResult<TerminalResultRecord> {
        let mut tx = self.pool.begin().await?;
    ...
    pub async fn ack_result_delivery(
        ...
        if changed == 1 {
            ...
            Err(StoreError::ResultDeliveryLeaseMismatch(delivery_id.to_string()))
        }
    }
    ...
    pub async fn fail_result_delivery(
        ...
        let state = if dead_letter { "dead_lettered" } else { "pending" };
...
mod tests {
    ...
    async fn sqlite_terminal_result_and_outbox_survive_restart() {
        ...
        store.migrate().await.unwrap();
        assert_eq!(
            store.get_terminal_result(&task_id).await.unwrap().encoded_result,
            b"terminal-result"
        );
    ...
    fn delivery_states_are_unique() {
        ...
        let labels = states.into_iter().map(ResultDeliveryState::as_str).collect::<HashSet<_>>();
        assert_eq!(labels.len(), 4);
    }
}
'''

write("proto/hermes/keryx/v1/result.proto", RESULT_PROTO)
write("crates/keryx-store/src/results.rs", RESULTS_RS)
...
# Existing tests use explicit InMemoryState literals. Extend the common one-line envelope tail.
text = read("crates/keryx-store/src/lib.rs")
text = re.sub(
    r"(\s+envelopes: HashMap::new\(\),\n)(\s+})",
    r"\1\2",
    text,
)
# Insert missing new fields before every explicit literal close that follows envelopes.
text = re.sub(
    r"(\s+envelopes: HashMap::new\(\),\n)(\s+})",
    r"\1\g<2>",
    text,
)
# More reliable block replacement for explicit test states.
text = text.replace(
...
print("Phase 17 protocol/store source applied")
