...
#[tokio::test]
async fn runtime_doctor_report_fails_when_startup_recovery_detects_corruption() {
    let dir = tempdir().unwrap();
    let data_dir = dir.path().join("corrupt-doctor-home");
    let db_path = data_dir.join("keryx.db");
    ...
    assert!(!doctor.healthy);
    assert_eq!(runtime.report().recovery.corruption_count(), 1);
    assert_eq!(
        runtime.report().recovery.corrupted_tasks,
        vec![record.task_id().clone()]
    );
    assert!(doctor.checks.iter().any(|check| {
        check.name == "startup_recovery"
            && !check.ready
            && check.detail.contains("corruption_count=1")
    }));
}
