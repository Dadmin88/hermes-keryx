...
`keryx-node` supervises its `ConnectNode` stream. Clean EOF, transport loss, and relay replacement reconnect automatically with authentication metadata reapplied and exponential backoff bounded from 250 ms to 5 seconds. Shutdown interrupts both an active stream and reconnect sleep. Transient daemon or acknowledgement failures remain unacknowledged and are retried after reconnect; verified terminal late results do not reopen the task or prevent later frames from being consumed.
